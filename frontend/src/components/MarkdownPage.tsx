...
export default function MarkdownPage({ mdFile }: { mdFile: string }) {
  ...
  useEffect(() => {
    ...
    setError(null)
    setContent(null)
    fetch(`${import.meta.env.BASE_URL}docs/${mdFile}`)
      .then((res) => {
  ...
  return (
    <div>
      ...
      <div style={{ maxWidth: '860px', margin: '0 auto', padding: '48px 56px' }} className="content-padding">
        {loading && (
          <div className="flex items-center gap-3 py-16" style={{ color: '#2E7D32' }}>
            <Loader2 size={20} className="animate-spin" />
            <span style={{ fontSize: '0.9rem' }}>Loading content\u2026</span>
          </div>
        )}
        {error && (
          <div className="flex items-start gap-3 py-16" style={{ color: '#c62828' }}>
            <AlertTriangle size={20} className="flex-shrink-0 mt-0.5" />
            <div>
              <p style={{ fontWeight: 600, margin: 0 }}>Could not load content</p>
              <p style={{ opacity: 0.7, marginTop: '4px', fontSize: '0.9rem' }}>{error}</p>
            </div>
          </div>
        )}
        {content && (
