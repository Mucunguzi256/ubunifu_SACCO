...
export default function Sidebar({ onClose }: { onClose: () => void }) {
  return (
    <nav className="w-full h-full overflow-y-auto flex flex-col">
      <div style={{ padding: '24px 20px 16px' }}>
        <div className="flex items-center gap-2 mb-1">
          ...
          <span style={{ fontFamily: "'Sora', sans-serif", fontSize: '1.15rem', fontWeight: 700, color: 'white', lineHeight: 1.2 }}>
            UBUNIFU SACCO
          </span>
        </div>
        <p style={{ fontSize: '0.72rem', color: 'rgba(255,255,255,0.6)', paddingLeft: '34px', margin: 0 }}>
          Documentation Portal
        </p>
      </div>

      <div style={{ height: '1px', background: 'rgba(255,255,255,0.1)', margin: '0 20px 8px' }} />

      <div style={{ padding: '0 12px 24px', flex: 1 }}>
        {navTree.map((item) => {
          if (item.divider) {
            return (
              <div key={item.divider} style={{ height: '1px', background: 'rgba(255,255,255,0.12)', margin: '8px 8px' }} />
            )
          }
          ...
          if (item.sectionHeader) {
            return (
              <div
                ...
                style={{
                  fontSize: '0.68rem', textTransform: 'uppercase', letterSpacing: '1.8px',
                  color: 'rgba(255,214,0,0.85)', fontFamily: "'Sora', sans-serif",
                  fontWeight: 700, padding: '8px 8px 4px', marginTop: '2px', userSelect: 'none',
                }}
              >
                {item.label}
              </div>
          ...
          return (
            <NavLink
              ...
              data-ocid={isHome ? 'nav.home.link' : `nav.${key}.link`}
              onClick={onClose}
              style={({ isActive }) => ({
                ...
                transition: 'all 0.2s ease', textDecoration: 'none', marginBottom: '1px',
              })}
              onMouseEnter={(e) => {
                const el = e.currentTarget as HTMLAnchorElement
                if (!el.getAttribute('aria-current')) el.style.background = 'rgba(255,255,255,0.1)'
              }}
              onMouseLeave={(e) => {
                const el = e.currentTarget as HTMLAnchorElement
